...
function init (env, ctx) {
  ...
  var rolesCollection = ctx.store ? ctx.store.collection(env.authentication_collections_prefix + 'roles') : null;
  var subjectsCollection = ctx.store ? ctx.store.collection(env.authentication_collections_prefix + 'subjects') : null;
  ...
  function create (collection) {
    function doCreate(obj, fn) {
      ...
      collection.insertOne(obj).then(function (result) {
        if (!obj._id && result.insertedId) {
  ...
  function remove (collection) {
    function doRemove (_id, callback) {
      collection.deleteOne({ '_id': new ObjectId(_id) }).then(function () {
        storage.reload(function loaded() {
          callback(null, null);
        });
      }).catch(function (err) {
  ...
  function save (collection) {
    function doSave (obj, callback) {
      ...
      collection.replaceOne({ _id: obj._id }, obj, { upsert: true }).then(function () {
