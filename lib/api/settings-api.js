 ...
 * PUT  /settings/display — update: update display, alarms, thresholds only
 * PUT  /settings/secret  — update: change API_SECRET (patient can change own password)
 */
function configure (app, wares, ctx, env) {
  ...
  api.put('/settings/display', ctx.authorization.isPermitted('api:settings:update'), function putDisplaySettings (req, res) {
    if (!ctx.settingsStore) {
      return res.status(503).json({ status: 503, message: 'Settings store not available' });
    }
  ...
  // PUT /settings/secret — change API_SECRET (authenticated users)
  api.put('/settings/secret', ctx.authorization.isPermitted('api:settings:update'), function putSecret (req, res) {
    if (!ctx.settingsStore) {
      return res.status(503).json({ status: 503, message: 'Settings store not available' });
    }

    var body = req.body;
    if (!body || !body.newSecret) {
      return res.status(400).json({ status: 400, message: 'Request body must contain "newSecret" field' });
    }

    ctx.settingsStore.changeApiSecret(body.newSecret, function (err) {
      if (err) {
        return res.status(400).json({ status: 400, message: err.message });
      }
      res.json({ status: 200, message: 'API_SECRET changed successfully. Existing tokens will continue to work until they expire.' });
    });
  });
