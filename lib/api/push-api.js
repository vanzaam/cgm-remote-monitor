...
function configure (app, wares, ctx) {
  ...
  // POST /push/received — ping-back from service worker confirming push delivery
  // No auth required — this is called from the SW which has no tokens
  api.post('/push/received', function pushReceived (req, res) {
    var body = req.body || {};
    console.log('Web Push: DELIVERY CONFIRMED on device |',
      'type:', body.type || '?', '|',
      'level:', body.level || '?', '|',
      'UA:', (body.ua || '?').substring(0, 50), '|',
      'at:', body.receivedAt || '?');
    res.json({ status: 200 });
  });
