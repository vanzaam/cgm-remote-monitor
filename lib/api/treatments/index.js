...
function configure (app, wares, ctx, env) {
  ...
  api.get('/treatments', function(req, res) {
    ...
    const inMemoryData = ctx.cache && ctx.cache.getData ? ctx.cache.getData('treatments') : [];
    const canServeFromMemory = inMemoryData.length >= query.count && Object.keys(query).length == 1 ? true : false;

    if (canServeFromMemory) {
      const sorted = _.sortBy(inMemoryData, function(item) {
        const mills = item.mills || (item.created_at ? new Date(item.created_at).getTime() : 0);
        return -mills;
      });
      serveTreatments(req, res, null, _take(sorted, query.count));
    } else {
