...
function startMultiTenant (tenantsConfigPath) {
  ...
  registry.bootAll(function onAllBooted (err) {
    ...
    // Tenant management API (on master app, not per-tenant)
    ...
    registry.tenants.forEach(function setupTenantWebsocket (tenant, hostname) {
      ...
      var websocket = require('./websocket')(tenant.env, tenant.ctx, nsp);
