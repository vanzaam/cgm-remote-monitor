...
function startMultiTenant (tenantsConfigPath) {
  ...
  registry.bootAll(function onAllBooted (err) {
    ...
    // Tenant reload endpoint — must be before tenantDispatch
    masterApp.post('/api/v1/tenant/reload', bodyParser.json(), function (req, res) {
      ...
      // Verify admin authorization via the tenant's own auth
      ...
      tenant.ctx.authorization.resolve({ api_secret: secret, token: null, ip: req.ip }, function (err, auth) {
        ...
        // Disconnect existing websocket namespace
