...
function config () {
  ...
  env.debug = {
    minify: readENVTruthy('DEBUG_MINIFY', false)
  };
...
function setAPISecret () {
  ...
  if (useSecret) {
    if (readENV('API_SECRET').length < consts.MIN_PASSPHRASE_LENGTH) {
      ...
      if (env.storageURI) {
        // Simple check if API_SECRET is in the URI (basic security check)
        if (env.storageURI.includes(apiSecret)) {
...
function setStorage () {
  env.storageURI = readENV('STORAGE_URI') || readENV('MONGODB_URI') || readENV('MONGO_CONNECTION') || readENV('MONGO') || readENV('MONGOLAB_URI') || readENV('CUSTOMCONNSTR_mongo');
