...
function init (env, ctx, server, rootIo, tenantHostname) {
  ...
  function start () {
    // Support both modes:
    // 1. Single-tenant: server is an HTTP server — create socket.io
    // 2. Multi-tenant: server is a socket.io Namespace — use directly
    if (server && typeof server.of === 'function' && typeof server.server !== 'undefined') {
      // This is a socket.io Namespace object (has .of on parent, has .server)
      io = server;
    } else if (server && typeof server.emit === 'function' && typeof server.on === 'function' && !server.listen) {
      // This is a socket.io Namespace (duck-typing: has emit/on but no listen)
      io = server;
    } else {
      // This is an HTTP server — create socket.io instance (original behavior)
  ...
  function listeners () {
    // io.on('connection') works for both Server and Namespace objects
    io.on('connection', function onConnection (socket) {
      ...
      socket.on('dbUpdate', function dbUpdate (data, callback) {
        ...
        try {
          ...
          console.error('Invalid _id in dbUpdate', data && data._id, err);
          if (callback) {
            callback({ result: 'Invalid _id' });
          }
          return;
        }
        ...
        ctx.store.collection(collection).updateOne({ '_id': id }
          , { $set: data.data }
            ...
            return ctx.store.collection(collection).findOne({ '_id': id });
            ...
            if (result !== null) {
              ctx.bus.emit('data-update', {
                ...
                , op: 'update'
                , changes: ctx.ddata.processRawDataForRuntime([result])
              });
            }
        }).catch(function(err) {
      ...
      socket.on('dbUpdateUnset', function dbUpdateUnset (data, callback) {
        ...
        var objId;
        try {
          objId = new ObjectId(data._id);
        } catch (err) {
          console.error('Invalid _id in dbUpdateUnset', data && data._id, err);
          if (callback) {
            callback({ result: 'Invalid _id' });
          }
          return;
        ...
        ctx.store.collection(collection).updateOne({ '_id': objId }, { $unset: data.data })
          .then(function() {
            return ctx.store.collection(collection).findOne({ '_id': objId });
          }).then(function(result) {
            console.log('Got results', result);
            if (result !== null) {
              ctx.bus.emit('data-update', {
            ...
            console.error('Error in dbUpdateUnset', err);
          });
      ...
      socket.on('dbAdd', function dbAdd (data, callback) {
        ...
        if (data.collection === 'treatments') {
          ...
          ctx.store.collection(collection).find(query).toArray().then(function (array) {
            ...
            return ctx.store.collection(collection).find(query_similiar).toArray().then(function (array) {
              ...
              return ctx.store.collection(collection).insertOne(data.data).then(function (doc) {

                if (!data.data._id && doc.insertedId) {
                  data.data._id = doc.insertedId;
                }

                ctx.bus.emit('data-update', {
                  type: data.collection
                  , op: 'update'
                  , changes: ctx.ddata.processRawDataForRuntime([data.data])
                });
            ...
            console.error(err);
            // Buffer treatment to disk on MongoDB failure
            if (ctx.diskBuffer) {
              ctx.diskBuffer.append(data.collection, data.data);
              ctx.bus.emit('data-update', {
          ...
          // try to find exact match
          ctx.store.collection(collection).find(queryDev).toArray().then(function (array) {
            ...
            console.error('devicestatus insertion error: ', err.message);
            if (ctx.diskBuffer) {
              ctx.diskBuffer.append('devicestatus', data.data);
              ctx.bus.emit('data-update', {
                type: 'devicestatus', op: 'update',
                changes: ctx.ddata.processRawDataForRuntime([data.data])
              });
            }
          });
        } else {
          ctx.store.collection(collection).insertOne(data.data).then(function (doc) {
      ...
      socket.on('dbRemove', function dbRemove (data, callback) {
        ...
        try {
          ...
          console.error('Invalid _id in dbRemove', data && data._id, err);
          if (callback) {
            callback({ result: 'Invalid _id' });
          }
          return;
        ...
        ctx.store.collection(collection).deleteOne({ '_id': objId })
          .then(function(stat) {
            ...
            console.error('Error in dbRemove', err);
          });
  ...
  function update () {
    ...
    lastData = ctx.ddata.clone ? ctx.ddata.clone() : JSON.parse(JSON.stringify(ctx.ddata));
  };

  start();
  listeners();

  // In multi-tenant mode, storageSocket/alarmSocket need an object with .of()
  // to create sub-namespaces scoped to this tenant
  var socketIoForApi3 = io;
  if (rootIo && tenantHostname) {
    socketIoForApi3 = {
      of: function (namespace) {
        return rootIo.of('/' + encodeURIComponent(tenantHostname) + namespace);
      }
    };
  }

  if (ctx.storageSocket) {
    ctx.storageSocket.init(socketIoForApi3);
  }

  if (ctx.alarmSocket) {
    ctx.alarmSocket.init(socketIoForApi3);
  }
