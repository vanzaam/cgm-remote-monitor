...
function buildEnvForTenant (tenantConfig) {
  ...
  // API Secret — generate random one if not provided (for demo/simulator mode)
  var apiSecret = tenantConfig.apiSecret || readENV('API_SECRET');
  if (!apiSecret && !tenantConfig.mongoUri) {
    apiSecret = require('crypto').randomBytes(12).toString('hex');
    console.log('[' + (tenantConfig.hostname || 'tenant') + '] Generated random API_SECRET for demo mode');
  }
  env.api_secret = null;
  ...
  // Multi-tenant: browser must connect socket.io to this path (see app.js + views/index.html)
  env.multiTenantSocketNamespace = '/' + encodeURIComponent(tenantConfig.hostname);
