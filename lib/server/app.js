...
function create (env, ctx) {
  ...
  app.locals.socketIoNamespace = env.multiTenantSocketNamespace || '';
  ...
  app.get("/sw.js", (req, res) => {
    // Service worker must NEVER be cached by browser — Chrome checks for updates
    // every 24h, but only if the server returns a different response
  ...
  if (process.env.NODE_ENV !== 'development') {
    ...
    if (env.debug && env.debug.minify) {
      ...
      console.log('Production environment detected, Minify disabled');
    }
