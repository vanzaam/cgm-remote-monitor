...
function init (env, ctx) {
  ...
  function tick () {
    ...
    // Also write to disk buffer (for persistence)
    if (ctx.diskBuffer) {
      ctx.diskBuffer.append('entries', entry);
    }
    ...
    if (ctx.diskBuffer) {
      ctx.diskBuffer.append('devicestatus', ds);
    }
    ...
    TREATMENT_EVENTS.forEach(function (te) {
      if (Math.abs(minuteRounded - te.t) < 3 && !te._fired) {
        ...
        if (ctx.diskBuffer) {
          ctx.diskBuffer.append('treatments', treatment);
        }

        // Reset fired flag after cycle point passes
        setTimeout(function () { te._fired = false; }, 5 * 60 * 1000);
      }
    });

    // Emit data-update so plugins/alarms process it
    if (ctx.bus) {
