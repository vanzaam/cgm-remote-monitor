...
function boot (env, language) {
  ...
  function checkSettings (ctx, next) {
    ...
    var simulatorMode = process.env.SIMULATOR === 'true' || process.env.SIMULATOR === '1';
    if (!env.storageURI && !simulatorMode) {
      ctx.bootErrors.push({'desc': 'Mandatory setting missing',
      err: 'MONGODB_URI setting is missing, cannot connect to database'});
    } else if (!env.storageURI && simulatorMode) {
      console.log('MONGODB_URI not set, but SIMULATOR mode enabled — continuing without database');
      ctx.simulatorMode = true;
    }
  ...
  function setupStorage (ctx, next) {
    ...
    if (!env.storageURI) {
      console.log('No storage URI configured — running without database');
      ctx.store = null;
      return next();
    }
  ...
  function loadSettingsFromDB (ctx, next) {

    console.log('Executing loadSettingsFromDB');

    if (hasBootErrors(ctx)) {
      return next();
    }

    var settingsStore = require('./settings-store')(env, ctx);
    ctx.settingsStore = settingsStore;

    settingsStore.loadFromDB(function (err) {
      if (err) {
        console.warn('Failed to load settings from DB, using defaults:', err.message);
      }
      next();
    });
  }
  ...
  function setupAuthorization (ctx, next) {
    ...
    if (!ctx.store) {
      // No database — authorization works with defaults only (API_SECRET based)
      console.log('Authorization: no database, using API_SECRET auth only');
      return next();
    }
  ...
  function setupInternals (ctx, next) {
    ...
    ctx.webPush = require('./webpush')(env, ctx);
    ...
    ctx.diskBuffer = require('./diskbuffer')(env, ctx);
    ctx.simulator = require('./simulator')(env, ctx);
  ...
  function ensureIndexes (ctx, next) {
    ...
    if (hasBootErrors(ctx) || !ctx.store) {
      return next();
    }
  ...
  function setupListeners (ctx, next) {
    ...
    // Web Push notifications — send browser push alongside Pushover/Maker
    if (ctx.webPush) {
      ctx.webPush.setupVAPID(function () {
        ctx.webPush.ensureIndexes();
        ctx.bus.on('notification', ctx.webPush.emitNotification);
        console.log('Web Push: listening for notification events');
      });
    }
  ...
  function finishBoot (ctx, next) {
    ...
    // Flush disk buffer if there's data from a previous crash/outage
    if (ctx.diskBuffer && ctx.diskBuffer.hasData()) {
      var bufferedCount = ctx.diskBuffer.count();
      console.log('Disk buffer: found', bufferedCount, 'documents from previous session, flushing...');
      ctx.diskBuffer.flush(function (err, flushed) {
        if (err) {
          console.warn('Disk buffer: flush on boot failed:', err.message, '— will retry on next data-received');
        } else {
          console.log('Disk buffer: flushed', flushed, 'documents on boot');
        }
      });
    }

    // Flush disk buffer when MongoDB recovers after an outage
    if (ctx.bus && ctx.diskBuffer) {
      ctx.bus.on('mongo-recovered', function () {
        if (ctx.diskBuffer.hasData()) {
          console.log('Disk buffer: MongoDB recovered, flushing buffered data...');
          ctx.diskBuffer.flush();
        }
      });

      // Trim buffer every hour to enforce 24h retention
