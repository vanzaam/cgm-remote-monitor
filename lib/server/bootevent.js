...
function boot (env, language) {
  ...
  function checkSettings (ctx, next) {
    ...
    var multiTenantMode = !!process.env.TENANTS_CONFIG;
    if (!env.storageURI && !simulatorMode && !multiTenantMode) {
      ctx.bootErrors.push({'desc': 'Mandatory setting missing',
      err: 'MONGODB_URI setting is missing, cannot connect to database'});
    } else if (!env.storageURI) {
      console.log('No MONGODB_URI — using in-memory storage');
    }
  ...
  function setupStorage (ctx, next) {
    ...
    if (!env.storageURI) {
      console.log('No storage URI configured — using in-memory storage');
      require('../storage/memory-storage')(env, function ready (err, store) {
        ctx.store = store;
        next();
      });
      return;
