...
function storage (collection, ctx) {
  ...
  function create (statuses, fn) {
    ...
    const insertPromises = statuses.map(function insertStatus (obj) {
      ...
      if (!col) {
        ...
        if (ctx.diskBuffer) ctx.diskBuffer.append('devicestatus', obj);
        r.push(obj);
        ctx.bus.emit('data-update', { type: 'devicestatus', op: 'update', changes: ctx.ddata.processRawDataForRuntime([obj]) });
        return Promise.resolve();
      }
      return col.insertOne(obj).then(function(results) {
  ...
  function list (opts, fn) {
    ...
    var col = api();
    if (!col) {
      if (ctx.ddata && ctx.ddata.devicestatus) return fn(null, ctx.ddata.devicestatus);
      return fn(null, []);
    }
    limit.call(col
      .find(query_for(opts))
