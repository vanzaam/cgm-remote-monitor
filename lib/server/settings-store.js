...
/** Mask returned by getForAPI — must not be persisted or applied as a real secret */
var SECRET_PLACEHOLDER = '[CONFIGURED]';

/**
 * Admin UI sends masked secrets back unchanged on save; strip them so merge keeps DB values.
 */
function stripSecretPlaceholdersFromPartial (partial) {
  if (!partial || typeof partial !== 'object') return partial;
  var p = _.cloneDeep(partial);
  if (p.auth && p.auth.API_SECRET === SECRET_PLACEHOLDER) {
    delete p.auth.API_SECRET;
  }
  if (p.integrations && typeof p.integrations === 'object') {
    _.forIn(p.integrations, function (plugin, name) {
      if (!plugin || typeof plugin !== 'object') return;
      if (plugin.password === SECRET_PLACEHOLDER) delete plugin.password;
      if (name === 'loop' && plugin.apnsKey === SECRET_PLACEHOLDER) delete plugin.apnsKey;
    });
  }
  return p;
}
...
/**
 * Recursively remove dangerous keys to prevent prototype pollution.
 */
...
/**
 * Validate threshold values are in reasonable BG ranges (mg/dl).
 */
function validateThresholds (thresholds) {
  ...
  checkRange('bgHigh', thresholds.bgHigh, 100, 600);
  checkRange('bgTargetTop', thresholds.bgTargetTop, 80, 400);
  checkRange('bgTargetBottom', thresholds.bgTargetBottom, 40, 200);
  checkRange('bgLow', thresholds.bgLow, 20, 120);
...
function init (env, ctx) {
  ...
  // Expose validators for use by API layer
  settingsStore.validateThresholds = validateThresholds;
  settingsStore.sanitizeObject = sanitizeObject;
  ...
  settingsStore.loadFromDB = function loadFromDB (callback) {
    ...
    col.findOne({ _id: SETTINGS_DOC_ID })
      .then(function (doc) {
        if (!doc) {
  ...
  settingsStore.save = function save (settingsDoc, callback) {
    ...
    col.replaceOne({ _id: SETTINGS_DOC_ID }, settingsDoc, { upsert: true })
      .then(function () {
      ...
      .catch(function (err) {
        // Buffer settings to disk on MongoDB failure
        if (ctx.diskBuffer) {
          ctx.diskBuffer.append(CONFIG_COLLECTION, settingsDoc);
          // Still apply to env so current process uses new settings
          applySettingsToEnv(settingsDoc);
          if (ctx.bus) ctx.bus.emit('settings-changed');
          callback(null, settingsDoc);
        } else {
   ...
   * Input is sanitized to prevent prototype pollution.
   */
  settingsStore.update = function update (partialDoc, callback) {
    ...
    var safeDoc = sanitizeObject(partialDoc);

    // Validate thresholds if provided
    if (safeDoc.thresholds) {
      var thresholdErrors = validateThresholds(safeDoc.thresholds);
      if (thresholdErrors) {
        return callback(new Error('Invalid thresholds: ' + thresholdErrors.join(', ')));
      }
    }

    // Validate API_SECRET if provided
    if (safeDoc.auth && safeDoc.auth.API_SECRET && safeDoc.auth.API_SECRET !== SECRET_PLACEHOLDER) {
      if (safeDoc.auth.API_SECRET.length < MIN_SECRET_LENGTH) {
        return callback(new Error('API_SECRET must be at least ' + MIN_SECRET_LENGTH + ' characters'));
      }
    }

    col.findOne({ _id: SETTINGS_DOC_ID })
      .then(function (existing) {
        var partial = stripSecretPlaceholdersFromPartial(safeDoc);
        var merged = _.merge({}, existing || {}, partial);
  ...
  settingsStore.getForAPI = function getForAPI (level, callback) {
    ...
    col.findOne({ _id: SETTINGS_DOC_ID })
      .then(function (doc) {
        ...
        if (doc.alarmTypes) result.alarmTypes = doc.alarmTypes;
        ...
        if (level === 'admin') {
          ...
          if (doc.integrations) {
            ...
            if (result.integrations.loop && result.integrations.loop.apnsKey) {
              result.integrations.loop.apnsKey = SECRET_PLACEHOLDER;
            }
            // Mask passwords
            _.forIn(result.integrations, function (plugin) {
              if (plugin && plugin.password) plugin.password = SECRET_PLACEHOLDER;
            });
          }
          ...
          if (doc.auth) {
            ...
            if (result.auth.API_SECRET) {
              result.auth.API_SECRET = SECRET_PLACEHOLDER;
            }
          }
        }
  ...
  function applySettingsToEnv (doc) {
    ...
    // Alarm types
    if (doc.alarmTypes) {
      env.settings.alarmTypes = Array.isArray(doc.alarmTypes) ? doc.alarmTypes : [doc.alarmTypes];
    }

    // Auth
    if (doc.auth) {
      if (doc.auth.API_SECRET && doc.auth.API_SECRET !== SECRET_PLACEHOLDER) {
    ...
    if (doc.integrations) {
      _.forIn(doc.integrations, function (pluginSettings, pluginName) {
        if (pluginSettings && typeof pluginSettings === 'object') {
          var toMerge = _.cloneDeep(pluginSettings);
          if (toMerge.password === SECRET_PLACEHOLDER) delete toMerge.password;
          if (pluginName === 'loop' && toMerge.apnsKey === SECRET_PLACEHOLDER) delete toMerge.apnsKey;
          env.extendedSettings[pluginName] = _.merge(env.extendedSettings[pluginName] || {}, toMerge);
        }
      });
    }
  }
  ...
  /**
   * Change API_SECRET — available to authenticated users.
   * Validates length and stores in MongoDB + applies to enclave.
   */
  settingsStore.changeApiSecret = function changeApiSecret (newSecret, callback) {
    if (!newSecret || newSecret.length < MIN_SECRET_LENGTH) {
      return callback(new Error('API_SECRET must be at least ' + MIN_SECRET_LENGTH + ' characters'));
    }

    settingsStore.update({ auth: { API_SECRET: newSecret } }, function (err, result) {
      if (err) return callback(err);
      console.log('API_SECRET changed via settings API');
      callback(null, result);
    });
  };
