...
function storage (env, ctx) {
  ...
  function create (obj, fn) {
    ...
    var col = api();
    if (!col) {
      if (ctx.diskBuffer) ctx.diskBuffer.append(env.activity_collection, obj);
      return fn(null, [obj]);
    }
    col.insertOne(obj).then(function () {
      ...
      if (ctx.diskBuffer) {
        ctx.diskBuffer.append(env.activity_collection, obj);
        fn(null, [obj]);
      } else {
        fn(err.message, null);
      }
  ...
  function save (obj, fn) {
    ...
    col2.replaceOne({ _id: obj._id }, obj, { upsert: true }).then(function () {
      fn(null, obj);
    }).catch(function (err) {
      if (ctx.diskBuffer) {
        ctx.diskBuffer.append(env.activity_collection, obj);
  ...
  function list(opts, fn) {
    ...
    var col = api();
    if (!col) {
      if (ctx.ddata && ctx.ddata.activity) return fn(null, ctx.ddata.activity);
      return fn(null, []);
    }
    limit.call(col
        .find(query_for(opts))
