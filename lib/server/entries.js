...
function storage (env, ctx) {
  ...
  function list (opts, fn) {
    ...
    var col = api();
    if (!col) {
      // No database — serve from in-memory cache
      if (ctx.ddata && ctx.ddata.sgvs) {
        return fn(null, [].concat(ctx.ddata.sgvs || [], ctx.ddata.mbgs || [], ctx.ddata.cals || []));
      }
      return fn(null, []);
    }
    limit.call(col
      ...
      if (ctx.ddata && ctx.ddata.sgvs && ctx.ddata.sgvs.length > 0) {
        console.log('Entries list: MongoDB unavailable, serving from in-memory cache (' + ctx.ddata.sgvs.length + ' entries)');
        var combined = [].concat(ctx.ddata.sgvs || [], ctx.ddata.mbgs || [], ctx.ddata.cals || []);
        fn(null, combined);
      } else {
        fn(err, null);
      }
  ...
  function remove (opts, fn) {
    api().deleteMany(query_for(opts)).then(function(stat) {
  ...
  function create (docs, fn) {
    ...
    docs.forEach(function(doc) {
      ...
      var col = api();
      if (!col) {
        // No database — buffer to disk and update in-memory
        if (ctx.diskBuffer) ctx.diskBuffer.append('entries', doc);
        ctx.bus.emit('data-update', {
          type: 'entries', op: 'update',
      ...
      col.replaceOne(query, doc, { upsert: true }).then(function(updateResults) {
        ...
        if (ctx.diskBuffer) {
          ctx.diskBuffer.append('entries', doc);
          // Still update in-memory data so WebSocket clients see the entry
          ctx.bus.emit('data-update', {
            type: 'entries'
            , op: 'update'
            , changes: ctx.ddata.processRawDataForRuntime([doc])
  ...
  function getEntry (id, fn) {
    api().findOne({ _id: new ObjectId(id) }).then(function(entry) {
      fn(null, entry);
    }).catch(function(err) {
      fn(err);
    });
