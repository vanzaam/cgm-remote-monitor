...
function init () {
  ...
  ddata.processRawDataForRuntime = (data) => {
    ...
    // Mutate in-place instead of _.cloneDeep to avoid massive allocations
    Object.keys(data).forEach(key => {
      if (typeof data[key] === 'object' && data[key]) {
        if (Object.prototype.hasOwnProperty.call(data[key], '_id')) {
          data[key]._id = data[key]._id.toString();
        }
        if (Object.prototype.hasOwnProperty.call(data[key], 'created_at')
            && !Object.prototype.hasOwnProperty.call(data[key], 'mills')) {
          data[key].mills = new Date(data[key].created_at).getTime();
        }
        if (Object.prototype.hasOwnProperty.call(data[key], 'sysTime')
            && !Object.prototype.hasOwnProperty.call(data[key], 'mills')) {       
          data[key].mills = new Date(data[key].sysTime).getTime();
        }
      }
    });
  ...
  ddata.idMergePreferNew = (oldData, newData) => {
    ...
    // Map-based O(n) merge instead of O(n²) nested loops
    const mergedMap = new Map();
    for (let i = 0; i < oldData.length; i++) {
      const item = oldData[i];
      if (item._id) mergedMap.set(String(item._id), item);
    }
    // New data overwrites old data (prefer new)
