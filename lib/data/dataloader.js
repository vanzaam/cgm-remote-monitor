...
function mergeProcessSort(oldData, newData, ageLimit) {
  ...
  // Map-based O(n) merge instead of O(n²) nested loops, preferring new data
...
function loadProfileSwitchTreatments(ddata, ctx, callback) {
    var dateRange = {
        $gte: new Date(ddata.lastUpdated - (constants.ONE_DAY * 30)).toISOString()
    };
...
function loadLatestSingle(ddata, ctx, dataType, callback) {
    ...
    var dateRange = {
        $gte: new Date(ddata.lastUpdated - (constants.ONE_DAY * 31)).toISOString()
    };
...
function loadProfile(ddata, ctx, callback) {
    ctx.profile.last(function(err, results) {
        if (!err && results) {
            ...
            // Cache profile to disk for MongoDB-free operation
            if (ctx.diskBuffer && profiles.length > 0) {
                ctx.diskBuffer.cacheWrite('last_profile', profiles[0]);
            }
        } else if (err && (!ddata.profiles || ddata.profiles.length === 0)) {
            // MongoDB failed and no profile in memory — load from disk cache
            if (ctx.diskBuffer) {
                var cached = ctx.diskBuffer.cacheRead('last_profile');
                if (cached) {
                    console.log('Profile: MongoDB unavailable, loaded from disk cache');
                    ddata.profiles = [cached];
                }
            }
        }
        callback();
...
function loadDatabaseStats(ddata, ctx, callback) {
    if (!ctx.store || !ctx.store.db) {
        ddata.dbstats = { dataSize: 0, indexSize: 0 };
        return callback();
    }
    ctx.store.db.stats().then(function (result) {
        if (result) {
            ddata.dbstats = { 
