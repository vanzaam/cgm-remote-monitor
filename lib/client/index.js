...
var alarmHelpers = require('./alarm-helpers');

var client = {};

client.alarmHelpers = {
  isAlarmForHigh: alarmHelpers.isAlarmForHigh,
  isAlarmForLow: alarmHelpers.isAlarmForLow,
  isWarningAlarmEnabledForSettings: alarmHelpers.isWarningAlarmEnabledForSettings,
  isUrgentAlarmEnabledForSettings: alarmHelpers.isUrgentAlarmEnabledForSettings
};
...
client.load = function load (serverSettings, callback) {
  ...
  function playAlarm (audio) {
    ...
    if (client.browserUtils.queryParms().mute !== 'true') {
      ...
      if (playing && _.isFunction(playing.catch)) {
        playing.catch(function onPlayError (err) {
          console.info('Unable to autoplay alarm audio:', err && err.name ? err.name : err);
        });
      }
    } else {
  ...
  function stopAlarm (isClient, silenceTime, notify) {
    ...
    $('#alarmSnoozeBtn').hide();
    $('#toolbar').removeClass('alarm-active');
    updateTitle();

    silenceTime = _.isNumber(silenceTime) ? silenceTime : times.mins(5).msecs;
  ...
  function getActiveSnooze () {
    var now = Date.now();
    var best = null;
    Object.keys(clientAlarms).forEach(function (key) {
      /* eslint-disable-next-line security/detect-object-injection */ // verified false positive
      var a = clientAlarms[key];
      if (a.lastAckTime && now < a.lastAckTime + a.silenceTime) {
        var remaining = (a.lastAckTime + a.silenceTime) - now;
        if (!best || remaining > best.remaining) {
          best = { level: a.level, group: a.group, remaining: remaining, endsAt: a.lastAckTime + a.silenceTime };
        }
      }
    });
    return best;
  ...
  var MUTE_THRESHOLD_MINS = 20160; // 14 days — anything longer is treated as muted

  function isMutedSnooze (remaining) {
    return remaining >= times.mins(MUTE_THRESHOLD_MINS).msecs;
  }

  function getAllActiveSnoozes () {
    var now = Date.now();
    var groupMap = {};
    Object.keys(clientAlarms).forEach(function (key) {
      /* eslint-disable-next-line security/detect-object-injection */ // verified false positive
      var a = clientAlarms[key];
      if (a.lastAckTime && a.group && now < a.lastAckTime + a.silenceTime) {
        var remaining = (a.lastAckTime + a.silenceTime) - now;
        var existing = groupMap[a.group];
        if (!existing || remaining > existing.remaining) {
          groupMap[a.group] = { level: a.level, group: a.group, remaining: remaining, endsAt: a.lastAckTime + a.silenceTime, forever: isMutedSnooze(remaining) };
        }
      }
    });
    var snoozes = Object.keys(groupMap).map(function(g) { return groupMap[g]; });
    return snoozes.sort(function(a, b) { return a.remaining - b.remaining; });
  }

  function getAllMutedGroups () {
    var snoozes = getAllActiveSnoozes();
    return snoozes.filter(function(s) { return s.forever; });
  }

  function updateSnoozeBar () {
    if (typeof window === 'undefined') return;
    // Don't update snooze bar until clientAlarms are loaded
    if (!clientAlarms || Object.keys(clientAlarms).length === 0) return;
    
    var storage = Storages.localStorage;
    var showBar = storage.get('showSnoozeBar');
    if (showBar === false || showBar === 'false') {
      $('#snoozeBar').hide();
      return;
    }

    var allSnoozes = getAllActiveSnoozes();
    var snoozes = allSnoozes.filter(function(s) { return !s.forever; });
    var hasMuted = allSnoozes.some(function(s) { return s.forever; });

    var hasAnything = snoozes.length > 0 || hasMuted;
    if (!hasAnything) {
      $('#snoozeBar').hide();
      $('#snoozeIconNormal').show();
      $('#snoozeIconSnoozed').hide();
      return;
    }

    $('#snoozeIconNormal').hide();
    $('#snoozeIconSnoozed').show();
    ...
    var minSnooze = snoozes[0];
    var mins = Math.ceil(minSnooze.remaining / 60000);
    var endsDate = new Date(minSnooze.endsAt);
    var hh = endsDate.getHours();
    var mm = endsDate.getMinutes();
    var timeStr = (hh < 10 ? '0' : '') + hh + ':' + (mm < 10 ? '0' : '') + mm;

    var label = client.translate ? client.translate('Snoozed') : 'Snoozed';
    var groupCount = snoozes.length;
    var text = label + ' ' + mins + 'm (' + timeStr + ')';
    if (groupCount > 1) {
      text += ' [' + groupCount + ']';
    }
    $('#snoozeBarText').text(text);
    $('#snoozeBar').show();
  }
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  /* global io */
  var socketIoPath = (typeof window !== 'undefined' && window.NS_SOCKET_IO_PATH) ? window.NS_SOCKET_IO_PATH : null;
  if (socketIoPath) {
    client.socket = socket = io(socketIoPath, { transports: ['polling', 'websocket'] });
    client.alarmSocket = alarmSocket = io(socketIoPath + '/alarm', { multiplex: true, transports: ['polling', 'websocket'] });
  } else {
    client.socket = socket = io({ transports: ['polling', 'websocket'] });
    client.alarmSocket = alarmSocket = io('/alarm', { multiplex: true, transports: ['polling', 'websocket'] });
  }
  ...
  function isWarningAlarmEnabled (notify) {
    return client.alarmHelpers.isWarningAlarmEnabledForSettings(client.settings, notify);
  }

  function isUrgentAlarmEnabled (notify) {
    return client.alarmHelpers.isUrgentAlarmEnabledForSettings(client.settings, notify);
  }
  ...
  alarmSocket.on('alarm', function(notify) {
    console.info('alarm received from server', notify);
    var enabled = isWarningAlarmEnabled(notify);

    if (enabled) {
      console.log('Alarm raised!');
      generateAlarm(alarmSound, notify);
    } else {
      console.info('alarm was disabled locally', client.latestSGV ? client.latestSGV.mgdl : 'N/A', client.settings);
    }
    chart.update(false);
  });

  alarmSocket.on('urgent_alarm', function(notify) {
    console.info('urgent alarm received from server', notify);
    var enabled = isUrgentAlarmEnabled(notify);

    if (enabled) {
      console.log('Urgent alarm raised!');
      generateAlarm(urgentAlarmSound, notify);
    } else {
      console.info('urgent alarm was disabled locally', client.latestSGV ? client.latestSGV.mgdl : 'N/A', client.settings);
    }
    chart.update(false);
  });
  ...
  alarmSocket.on('clear_alarm', function(notify) {
    if (alarmInProgress) {
      ...
      stopAlarm(false, notify && notify.remainingMills, notify);
      return;
    }

    if (notify && _.isNumber(notify.level) && notify.group) {
      var alarm = getClientAlarm(notify.level, notify.group);
      alarm.lastAckTime = Date.now();
      alarm.silenceTime = _.isNumber(notify.remainingMills) ? notify.remainingMills : times.mins(5).msecs;
      if (alarm.group === 'Time Ago') {
        container.removeClass('alarming-timeago');
      }
    }
  ...
  */
  ...
  $('#snoozeBarCancel').click(function(event) {
    event.preventDefault();
    var snoozes = getAllActiveSnoozes();
    snoozes.forEach(function(s) {
      alarmSocket.emit('ack', s.level, s.group, 0);
      var alarm = getClientAlarm(s.level, s.group);
      if (alarm) {
        alarm.lastAckTime = 0;
        alarm.silenceTime = 0;
      }
  ...
  $('#snoozeBarExtend').click(function(event) {
    event.preventDefault();
    var snoozes = getAllActiveSnoozes();
    snoozes.forEach(function(s) {
      var newTime = s.remaining + times.mins(30).msecs;
      alarmSocket.emit('ack', s.level, s.group, newTime);
      var alarm = getClientAlarm(s.level, s.group);
      if (alarm) {
        alarm.lastAckTime = Date.now();
        alarm.silenceTime = newTime;
      }
    });
    updateSnoozeBar();
  });

  var GLUCOSE_GROUPS = ['Glucose Urgent High', 'Glucose High', 'Glucose Urgent Low', 'Glucose Low'];
  var SNOOZE_FOREVER_MINS = 525600; // 1 year

  var SNOOZE_BELL_OFF_SVG = '<svg class="snooze-bell-svg" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/><line x1="2" y1="2" x2="22" y2="22" stroke="#ff5252" stroke-width="2.5" stroke-linecap="round"/></svg>';

  function getKnownAlarmGroups () {
    var groups = ['Glucose Urgent High', 'Glucose High', 'Glucose Urgent Low', 'Glucose Low', 'Time Ago', 'Pump', 'OpenAPS', 'Loop'];
    Object.keys(clientAlarms).forEach(function(key) {
      /* eslint-disable-next-line security/detect-object-injection */ // verified false positive
      var a = clientAlarms[key];
  ...
  function buildSnoozeGroupItem (group, s, t) {
    var isActive = !!s;
    var isMuted = isActive && s.forever;
    var itemClass = isMuted ? 'muted' : (isActive ? 'active' : 'inactive');
    var timeText = '';
    if (isMuted) {
      timeText = t('Muted');
    } else if (isActive) {
      var mins = Math.ceil(s.remaining / 60000);
      var endsDate = new Date(s.endsAt);
      var hh = endsDate.getHours();
      var mm = endsDate.getMinutes();
      var timeStr = (hh < 10 ? '0' : '') + hh + ':' + (mm < 10 ? '0' : '') + mm;
      timeText = mins + 'm (' + timeStr + ')';
    } else {
      timeText = t('Not snoozed');
    }

    var groupName = t(group);
    var $item = $('<div></div>')
      .addClass('snooze-group-item')
      .addClass(itemClass)
      .attr('data-group', group);

    var $info = $('<div></div>');
    $info.append($('<div></div>').addClass('snooze-group-name').text(groupName));
    $info.append($('<div></div>').addClass('snooze-group-time').text(timeText));

    var $actions = $('<div></div>').addClass('snooze-group-actions');
    if (isMuted) {
      $actions.append(
        $('<a></a>')
          .attr('href', '#')
          .addClass('snooze-cancel-group')
          .addClass('cancel')
          .attr('data-group', group)
      ...
      $actions.append(
        $('<a></a>')
          .attr('href', '#')
          .addClass('snooze-forever-group')
          .addClass('snooze-forever-btn')
          .attr('data-group', group)
          .attr('title', t('Snooze forever'))
          .html(SNOOZE_BELL_OFF_SVG)
      );
    }
  ...
  function renderSnoozeManager () {
    var body = $('#snoozeManagerBody');
    body.empty();

    var t = client.translate || function(s) { return s; };

    var snoozes = getAllActiveSnoozes();
    var snoozedGroups = {};
    snoozes.forEach(function(s) {
      snoozedGroups[s.group] = s;
    });

    var allGroups = getKnownAlarmGroups();
    if (allGroups.length === 0) {
      body.append('<div class="snooze-no-alarms">' + t('No alarm groups') + '</div>');
      return;
    }

    var glucoseSnoozes = GLUCOSE_GROUPS.map(function(g) { return snoozedGroups[g]; }).filter(Boolean);
    var glucoseAllSnoozed = glucoseSnoozes.length === GLUCOSE_GROUPS.length;
    var glucoseAnySnoozed = glucoseSnoozes.length > 0;
    var glucoseAllMuted = glucoseAllSnoozed && glucoseSnoozes.every(function(s) { return s.forever; });
    var glucoseAnyMuted = glucoseSnoozes.some(function(s) { return s.forever; });
    var glucoseAnyTimed = glucoseSnoozes.some(function(s) { return !s.forever; });
    var glucoseHeaderClass = glucoseAllMuted ? 'muted' : (glucoseAllSnoozed ? 'active' : (glucoseAnySnoozed ? 'partial' : 'inactive'));

    var glucoseTimeText;
    if (glucoseAllMuted) {
      glucoseTimeText = t('Muted');
    } else if (glucoseAnyTimed) {
      var timedSnoozes = glucoseSnoozes.filter(function(s) { return !s.forever; });
      var minSnooze = timedSnoozes.reduce(function(a, b) { return a.remaining < b.remaining ? a : b; });
      var gMins = Math.ceil(minSnooze.remaining / 60000);
      var gDate = new Date(minSnooze.endsAt);
      var gHH = gDate.getHours();
      var gMM = gDate.getMinutes();
      var gTimeStr = (gHH < 10 ? '0' : '') + gHH + ':' + (gMM < 10 ? '0' : '') + gMM;
      glucoseTimeText = gMins + 'm (' + gTimeStr + ')';
      if (!glucoseAllSnoozed) {
        glucoseTimeText += ' (' + glucoseSnoozes.length + '/' + GLUCOSE_GROUPS.length + ')';
      }
    } else if (glucoseAnyMuted) {
      glucoseTimeText = t('Muted') + ' (' + glucoseSnoozes.length + '/' + GLUCOSE_GROUPS.length + ')';
    } else {
      glucoseTimeText = t('Not snoozed');
    }

    var isCollapsed = Storages.localStorage.get('glucoseSubgroupCollapsed') !== false;
    var $subgroup = $('<div></div>').addClass('snooze-subgroup').toggleClass('collapsed', isCollapsed);

    var $header = $('<div></div>').addClass('snooze-subgroup-header').addClass(glucoseHeaderClass);
    var $titleWrap = $('<div></div>').addClass('snooze-subgroup-title');
    $titleWrap.append($('<span></span>').addClass('snooze-subgroup-name').text(t('Glucose')));
    $titleWrap.append($('<span></span>').addClass('snooze-subgroup-toggle').text('▾'));
    var $headerInfo = $('<div></div>')
      .append($titleWrap)
      .append($('<div></div>').addClass('snooze-subgroup-time').text(glucoseTimeText));

    var $headerActions = $('<div></div>').addClass('snooze-group-actions');
    if (glucoseAnySnoozed) {
      $headerActions.append(
        $('<a></a>')
          .attr('href', '#')
          .addClass('snooze-cancel-glucose')
          .addClass('cancel')
          .text(t('Cancel All'))
      );
    } else {
      [30, 60, 120].forEach(function(mins) {
        $headerActions.append(
          $('<a></a>')
            .attr('href', '#')
            .addClass('snooze-add-glucose')
            .attr('data-mins', mins)
            .text(mins === 30 ? '30m' : (mins === 60 ? '1h' : '2h'))
        );
      });
      $headerActions.append(
        $('<a></a>')
          .attr('href', '#')
          .addClass('snooze-forever-glucose')
          .addClass('snooze-forever-btn')
          .attr('title', t('Snooze forever'))
          .html(SNOOZE_BELL_OFF_SVG)
      );
    ...
    $header.append($headerInfo).append($headerActions);
    $header.on('click', function() {
      $subgroup.toggleClass('collapsed');
      Storages.localStorage.set('glucoseSubgroupCollapsed', $subgroup.hasClass('collapsed'));
    });

    var $children = $('<div></div>').addClass('snooze-subgroup-children');
    GLUCOSE_GROUPS.forEach(function(group) {
      $children.append(buildSnoozeGroupItem(group, snoozedGroups[group], t));
    });

    $subgroup.append($header).append($children);
    body.append($subgroup);

    var nonGlucoseGroups = allGroups.filter(function(g) { return GLUCOSE_GROUPS.indexOf(g) === -1; });
    nonGlucoseGroups.forEach(function(group) {
      body.append(buildSnoozeGroupItem(group, snoozedGroups[group], t));
    });
  }

  function openSnoozeManager () {
    var t = client.translate || function(s) { return s; };
    $('#snoozeManagerModal .snooze-modal-header span').text(t('Snooze Manager'));
    $('#snoozeAllBtn').text(t('Snooze All 30m'));
    $('#cancelAllSnoozesBtn').text(t('Cancel All'));
    renderSnoozeManager();
    $('#snoozeManagerModal').show();
  }

  function closeSnoozeManager () {
    $('#snoozeManagerModal').hide();
  }

  $('#snoozeBarText').click(function(event) {
    event.preventDefault();
    openSnoozeManager();
  });

  $('#snoozeManagerBtn').click(function(event) {
    event.preventDefault();
    openSnoozeManager();
  });

  $('#snoozeManagerClose').click(function(event) {
    event.preventDefault();
    closeSnoozeManager();
  });

  $('#snoozeManagerModal').click(function(event) {
    if (event.target === this) {
      closeSnoozeManager();
    }
  });

  $('#notificationSnooze').click(function(event) {
    event.preventDefault();
    event.stopPropagation();
    var notify = currentNotify;
    if (notify) {
      var snoozeTime = times.mins(30).msecs;
      alarmSocket.emit('ack', notify.level, notify.group, snoozeTime);
      stopAlarm(false, snoozeTime, notify);
    }
  });

  // Обновляем текст кнопки отложения при инициализации языка
  if (client.translate) {
    $('#notificationSnooze').text(client.translate('Snooze'));
    $('#alarmSnoozeBtn').text(client.translate('Snooze') + ' 30m');
  }

  $('#alarmSnoozeBtn').click(function(event) {
    event.preventDefault();
    var notify = currentNotify;
    if (notify) {
      var snoozeTime = times.mins(30).msecs;
  ...
  $(document).on('click', '.snooze-forever-group', function(event) {
    event.preventDefault();
    var group = $(this).data('group');
    var snoozeTime = times.mins(SNOOZE_FOREVER_MINS).msecs;
    alarmSocket.emit('ack', levels.URGENT, group, snoozeTime);
    alarmSocket.emit('ack', levels.WARN, group, snoozeTime);
    [levels.URGENT, levels.WARN].forEach(function(lvl) {
      var alarm = getClientAlarm(lvl, group);
      if (alarm) {
        alarm.lastAckTime = Date.now();
        alarm.silenceTime = snoozeTime;
      }
    });
    updateSnoozeBar();
    renderSnoozeManager();
  ...
  $(document).on('click', '.snooze-forever-glucose', function(event) {
    event.preventDefault();
    event.stopPropagation();
    var snoozeTime = times.mins(SNOOZE_FOREVER_MINS).msecs;
    GLUCOSE_GROUPS.forEach(function(group) {
  ...
  $(document).on('click', '.snooze-cancel-glucose', function(event) {
    event.preventDefault();
    event.stopPropagation();
    GLUCOSE_GROUPS.forEach(function(group) {
      alarmSocket.emit('ack', levels.URGENT, group, 0);
      alarmSocket.emit('ack', levels.WARN, group, 0);
      [levels.URGENT, levels.WARN].forEach(function(lvl) {
        var alarm = getClientAlarm(lvl, group);
        if (alarm) {
          alarm.lastAckTime = 0;
  ...
  $(document).on('click', '.snooze-add-glucose', function(event) {
    event.preventDefault();
    event.stopPropagation();
    var mins = $(this).data('mins') || 30;
    var snoozeTime = times.mins(mins).msecs;
    GLUCOSE_GROUPS.forEach(function(group) {
      alarmSocket.emit('ack', levels.URGENT, group, snoozeTime);
      var alarm = getClientAlarm(levels.URGENT, group);
      if (alarm) {
        alarm.lastAckTime = Date.now();
        alarm.silenceTime = snoozeTime;
      }
  ...
  $(document).on('click', '.snooze-cancel-group', function(event) {
    event.preventDefault();
    var group = $(this).data('group');
    var level = $(this).data('level');
    alarmSocket.emit('ack', level, group, 0);
    var alarm = getClientAlarm(level, group);
    if (alarm) {
      alarm.lastAckTime = 0;
      alarm.silenceTime = 0;
    }
  ...
  $(document).on('click', '.snooze-extend-group', function(event) {
    event.preventDefault();
    var group = $(this).data('group');
    var level = $(this).data('level');
    var snoozes = getAllActiveSnoozes();
    var s = snoozes.find(function(sn) { return sn.group === group; });
    if (s) {
      var newTime = s.remaining + times.mins(30).msecs;
      alarmSocket.emit('ack', level, group, newTime);
      var alarm = getClientAlarm(level, group);
      if (alarm) {
        alarm.lastAckTime = Date.now();
        alarm.silenceTime = newTime;
  ...
  $(document).on('click', '.snooze-add-group', function(event) {
    event.preventDefault();
    var group = $(this).data('group');
    var mins = $(this).data('mins') || 30;
    var level = levels.URGENT;
    var snoozeTime = times.mins(mins).msecs;
    alarmSocket.emit('ack', level, group, snoozeTime);
    var alarm = getClientAlarm(level, group);
    if (alarm) {
      alarm.lastAckTime = Date.now();
      alarm.silenceTime = snoozeTime;
  ...
  $('#snoozeAllBtn').click(function(event) {
    event.preventDefault();
    var groups = getKnownAlarmGroups();
    var snoozeTime = times.mins(30).msecs;
  ...
  $('#cancelAllSnoozesBtn').click(function(event) {
    event.preventDefault();
    var snoozes = getAllActiveSnoozes();
    snoozes.forEach(function(s) {
      alarmSocket.emit('ack', s.level, s.group, 0);
