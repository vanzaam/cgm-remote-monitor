...
client.load = function load (serverSettings, callback) {
    ...
    Object.keys(clientAlarms).forEach(function (key) {
      ...
      if (a.lastAckTime && a.group && now < a.lastAckTime + a.silenceTime) {
        ...
        if (!existing || remaining > existing.remaining) {
          groupMap[a.group] = { level: a.level, group: a.group, remaining: remaining, endsAt: a.lastAckTime + a.silenceTime };
        }
      }
    });
  ...
  */
  ...
  function buildSnoozeGroupItem (group, s, t) {
    var isActive = !!s;
    var itemClass = isActive ? 'active' : 'inactive';
    var timeText = '';
    if (isActive) {
  ...
  function renderSnoozeManager () {
    ...
    var glucoseHeaderClass = glucoseAllSnoozed ? 'active' : (glucoseAnySnoozed ? 'partial' : 'inactive');

    var glucoseTimeText;
    if (glucoseAnySnoozed) {
      var minSnooze = glucoseSnoozes.reduce(function(a, b) { return a.remaining < b.remaining ? a : b; });
