...
client.load = function load (serverSettings, callback) {
  ...
  alarmSocket.on('alarm', function(notify) {
    ...
    var isDefaultGroup = group === 'default';
    var title = notify && notify.title;
    var eventName = notify && notify.eventName;
    var isHigh = isDefaultGroup && (eventName === 'high' || (title && /\bHIGH\b/i.test(title)));
    var isLow = isDefaultGroup && (eventName === 'low' || (title && /\bLOW\b/i.test(title)));

    if (isDefaultGroup) {
      if (isHigh || isLow) {
        enabled = (isHigh && client.settings.alarmHigh) || (isLow && client.settings.alarmLow);
      } else {
        enabled = client.settings.alarmHigh || client.settings.alarmLow;
      }
    } else {
      enabled = client.settings.alarmHigh || client.settings.alarmLow;
    }
  ...
  alarmSocket.on('urgent_alarm', function(notify) {
    ...
    if (isDefaultGroup) {
      if (isHigh || isLow) {
        enabled = (isHigh && client.settings.alarmUrgentHigh) || (isLow && client.settings.alarmUrgentLow);
      } else {
        enabled = client.settings.alarmUrgentHigh || client.settings.alarmUrgentLow;
      }
    } else {
      enabled = client.settings.alarmUrgentHigh || client.settings.alarmUrgentLow;
    }
  ...
  */
  ...
  function getKnownAlarmGroups () {
    var groups = ['Time Ago', 'Pump', 'OpenAPS', 'Loop'];
    Object.keys(clientAlarms).forEach(function(key) {
      /* eslint-disable-next-line security/detect-object-injection */ // verified false positive
      var a = clientAlarms[key];
