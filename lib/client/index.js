...
client.load = function load (serverSettings, callback) {
  ...
  */
  ...
  function renderSnoozeManager () {
    ...
    if (groups.length === 0) {
      body.append('<div class="snooze-no-alarms">' + (client.translate ? client.translate('No alarm groups') : 'No alarm groups') + '</div>');
      return;
    }

    groups.forEach(function(group) {
      var s = snoozedGroups[group];
      var isActive = !!s;
      ...
      if (isActive) {
        ...
        timeText = client.translate ? client.translate('Not snoozed') : 'Not snoozed';
      }

      var groupName = client.translate ? client.translate(group) : group;
      var $item = $('<div></div>')
      ...
      if (isActive) {
        ...
        $actions.append(
          $('<a></a>')
            ...
            .attr('data-group', group)
            .attr('data-level', s.level)
            .text(client.translate ? client.translate('Cancel') : 'Cancel')
        );
      } else {
        $actions.append(
          $('<a></a>')
