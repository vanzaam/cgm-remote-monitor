...
client.load = function load (serverSettings, callback) {
  ...
  function isForeverSnooze (silenceTime) {
    return silenceTime >= times.mins(SNOOZE_FOREVER_MINS - 1).msecs;
  }
    ...
    Object.keys(clientAlarms).forEach(function (key) {
      ...
      if (a.lastAckTime && a.group && now < a.lastAckTime + a.silenceTime) {
        ...
        if (!existing || remaining > existing.remaining) {
          groupMap[a.group] = { level: a.level, group: a.group, remaining: remaining, endsAt: a.lastAckTime + a.silenceTime, forever: isForeverSnooze(a.silenceTime) };
        }
      }
    });
