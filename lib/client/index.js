...
client.load = function load (serverSettings, callback) {
  ...
  function updateSnoozeBar () {
    ...
    var snooze = getActiveSnooze();
    if (!snooze) {
      $('#snoozeBar').hide();
      return;
    }

    var mins = Math.ceil(snooze.remaining / 60000);
    var endsDate = new Date(snooze.endsAt);
    var hh = endsDate.getHours();
    var mm = endsDate.getMinutes();
    var timeStr = (hh < 10 ? '0' : '') + hh + ':' + (mm < 10 ? '0' : '') + mm;
    var label = client.translate ? client.translate('Alarms snoozed') : 'Alarms snoozed';
    $('#snoozeBarText').text(label + ' — ' + mins + 'm (' + timeStr + ')');
    $('#snoozeBar').show();
  }
  ...
  */
  ...
  $('#snoozeBarCancel').click(function(event) {
    ...
    var snooze = getActiveSnooze();
    if (snooze) {
      alarmSocket.emit('ack', snooze.level, snooze.group, 0);
    }
    updateSnoozeBar();
  });
  ...
  $('#snoozeBarExtend').click(function(event) {
    ...
    var snooze = getActiveSnooze();
    if (snooze) {
      var newTime = snooze.remaining + times.mins(30).msecs;
      alarmSocket.emit('ack', snooze.level, snooze.group, newTime);
    }
    updateSnoozeBar();
  });
