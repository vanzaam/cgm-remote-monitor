...
function init (env, ctx) {
  ...
  var canPersistSnoozes = !!(ctx && ctx.store && ctx.store.collection);
  var snoozeCollection = canPersistSnoozes ? ctx.store.collection('alarm_snoozes') : null;
  ...
  function loadPersistedSnoozes () {
    if (!canPersistSnoozes) {
      return;
    }

    snoozeCollection.find({}).toArray().then(function onDocs (docs) {
      var now = Date.now();
      docs.forEach(function eachDoc (doc) {
        if (!doc || !_.isNumber(doc.level) || !doc.group || !_.isNumber(doc.lastAckTime) || !_.isNumber(doc.silenceTime)) {
          return;
        }

        if (now >= doc.lastAckTime + doc.silenceTime) {
          snoozeCollection.deleteOne({ _id: doc._id }).catch(function onDeleteErr (err) {
            console.error('Unable to remove expired persisted snooze', err);
          });
          return;
        }

        var alarm = getAlarm(doc.level, doc.group);
        alarm.lastAckTime = doc.lastAckTime;
        alarm.silenceTime = doc.silenceTime;

        if (!doc.expiresAt) {
          snoozeCollection.updateOne(
            { _id: doc._id },
            { $set: { expiresAt: new Date(doc.lastAckTime + doc.silenceTime) } }
          ).catch(function onMigrateErr (err) {
            console.error('Unable to backfill expiresAt for persisted snooze', err);
          });
        }
      });

      if (docs.length) {
        console.info('restored ' + docs.length + ' persisted alarm snooze(s)');
      }
    }).catch(function onErr (err) {
      console.error('Unable to load persisted alarm snoozes', err);
    });
  }

  function persistAckState (alarm) {
    if (!canPersistSnoozes || !alarm) {
      return;
    }

    var id = alarm.level + '-' + alarm.group;
    var now = Date.now();
    var active = alarm.lastAckTime && now < alarm.lastAckTime + alarm.silenceTime;

    if (!active) {
      snoozeCollection.deleteOne({ _id: id }).catch(function onDeleteErr (err) {
        console.error('Unable to delete persisted snooze', err);
      });
      return;
    }
    ...
    var doc = {
      ...
      level: alarm.level,
      group: alarm.group,
      lastAckTime: alarm.lastAckTime,
      silenceTime: alarm.silenceTime,
      expiresAt: new Date(alarm.lastAckTime + alarm.silenceTime),
      updatedAt: new Date(now).toISOString()
    };

    snoozeCollection.replaceOne({ _id: id }, doc, { upsert: true }).catch(function onSaveErr (err) {
      console.error('Unable to persist snooze', err);
    });
  }

  function clearNotificationFor (level, group, remainingMills) {
    var notify = {
      clear: true
      , title: 'All Clear'
    ...
    if (_.isNumber(remainingMills)) {
      notify.remainingMills = Math.max(0, remainingMills);
    }
  ...
  if (canPersistSnoozes) {
    ctx.store.ensureIndexes(snoozeCollection, ['level', 'group', 'lastAckTime']);
    snoozeCollection.createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0, background: true }).catch(function onTtlIndexErr (err) {
      console.error('Unable to ensure TTL index for persisted snoozes', err);
    });
  ...
  notifications.ack = function ack (level, group, time, sendClear) {
    ...
    if (Date.now() < alarm.lastAckTime + alarm.silenceTime) {
      console.info('Alarm already snoozed, updating snooze time, level:', level, ', group:', group);
    }

    alarm.lastAckTime = Date.now();
    alarm.silenceTime = _.isNumber(time) ? time : THIRTY_MINUTES;
    delete alarm.lastEmitTime;
    persistAckState(alarm);
    ...
    */
    if (sendClear) {
      var notify = clearNotificationFor(level, group, alarm.silenceTime);
  ...
  notifications.getActiveSnoozes = function getActiveSnoozes (nowMills) {
    var now = nowMills || Date.now();

    return _.chain(alarms)
      .values()
      .filter(function eachAlarm (alarm) {
        return alarm.lastAckTime && now < alarm.lastAckTime + alarm.silenceTime;
      })
      .map(function eachSnoozedAlarm (alarm) {
        var remainingMills = (alarm.lastAckTime + alarm.silenceTime) - now;
        return clearNotificationFor(alarm.level, alarm.group, remainingMills);
      })
      .value();
  };
