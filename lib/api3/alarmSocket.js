...
function AlarmSocket (app, env, ctx) {
  ...
  self.subscribe = function subscribe (socket, message, returnCallback) {
    ...
    function emitActiveSnoozes () {
      if (!ctx.notifications || !ctx.notifications.getActiveSnoozes) {
        return;
      }

      const activeSnoozes = ctx.notifications.getActiveSnoozes();
      activeSnoozes.forEach(function eachSnooze (notify) {
        socket.emit('clear_alarm', notify);
      });

      if (activeSnoozes.length) {
        console.info(LOG + 'synced ' + activeSnoozes.length + ' active snooze(s) to client ' + socket.client.id);
      }
    }
