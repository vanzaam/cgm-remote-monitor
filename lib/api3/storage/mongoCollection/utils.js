...
function parseFilter (filterDef, logicalOperator, onlyValid) {
  ...
  for (const itemDef of filterDef) {
    ...
    switch (itemDef.operator) {
      ...
      case 'in':
        item = { $in: (itemDef.value == null ? '' : itemDef.value).toString().split('|').filter(Boolean) };
        break;

      case 'nin':
        item = { $nin: (itemDef.value == null ? '' : itemDef.value).toString().split('|').filter(Boolean) };
        break;

      case 're':
        item = { $regex: (itemDef.value == null ? '' : itemDef.value).toString() };
        break;
