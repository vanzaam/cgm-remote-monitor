...
function MongoCachedCollection (ctx, env, colName, baseStorage) {
  ...
  self.insertOne = async (doc) => {
    try {
      ...
      if (ctx.diskBuffer) {
        ctx.diskBuffer.append(colName, doc);
        if (cacheSupported()) {
          updateInCache([doc]);
        }
        if (doc._id) {
        ...
        return doc.identifier || 'buffered';
      }
      throw err;
    }
  }
  ...
  self.replaceOne = async (identifier, doc) => {
    try {
      ...
      if (ctx.diskBuffer) {
        doc._bufferedIdentifier = identifier;
        ctx.diskBuffer.append(colName, doc);
        if (cacheSupported()) {
          updateInCache([doc]);
        }
  ...
  self.updateOne = async (identifier, setFields) => {
    try {
      ...
      if (ctx.diskBuffer) {
        ctx.diskBuffer.append(colName, { _bufferedIdentifier: identifier, $set: setFields });
        return { updated: 0 };
      }
      throw err;
    }
