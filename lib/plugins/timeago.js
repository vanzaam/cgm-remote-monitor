...
function init(ctx) {
  ...
  timeago.checkStatus = function checkStatus(sbx) {
    ...
    if (isHibernationDetected()) {
      // Log hibernation detection with timestamp, but debounce to avoid console spam
      var nowTime = now.getTime();
      if (nowTime - lastHibernationLogTime > 5000) { // Log at most once per 5 seconds
        console.log('Hibernation detected at ' + now.toLocaleTimeString() + ', suspending timeago alarm (delta: ' + Math.round(delta / 1000) + 's)');
        lastHibernationLogTime = nowTime;
      }
      return 'current';
    }
