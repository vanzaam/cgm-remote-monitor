...
function init (ctx) {
  ...
  function glucoseGroupFor (prop, levels) {
    if (prop.eventName === 'high') {
      return prop.level === levels.URGENT ? 'Glucose Urgent High' : 'Glucose High';
    }
    if (prop.eventName === 'low') {
      return prop.level === levels.URGENT ? 'Glucose Urgent Low' : 'Glucose Low';
    }
    return undefined;
  }
  ...
  ar2.checkNotifications = function checkNotifications (sbx) {
    ...
    if (prop && prop.level) {
      const notify = {
        ...
        , group: glucoseGroupFor(prop, sbx.levels)
